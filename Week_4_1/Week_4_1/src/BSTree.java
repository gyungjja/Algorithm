...
public class BSTree {
	...
	public void deleteBST(BSTNode root,int key)
	{
		...
		else 
		{
			...
			if(p.getRight() == q)	//삭제하려는 값이 오른쪽값의 최소값과 같으면
				p.setRight(q.getRight());	//최소값으로 설정
	...
	public boolean split(int key, BSTree bBST, BSTree cBST)
	{
		...
		BSTNode p = this.treeCopy(this.root);	//복사 노드
		BSTNode s = small;	//small 노드
		BSTNode l = large;	//large 노드
		while(p != null)	//비어있지 않을때 까지 
		{
		...
		if(key > root.getKey())	//찾으려는 값이 루트의 값보다 크면
			s.setRight(null);	//스몰노드 오른쪽 널 설정
		else if(key < root.getKey())//찾으려는 값이 루트의 값보다 작으면
			l.setLeft(null);	//라지노드 왼쪽 널 설정
