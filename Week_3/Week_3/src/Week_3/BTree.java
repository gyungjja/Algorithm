...
public class BTree {
	...
	private TreeNode treeCopy(TreeNode T){	//이진 트리 Copy
		TreeNode S = null;	//공백 이진 트리 초기화
		if(T != null) {		//T 가 널이 아니면
			TreeNode L = treeCopy(T.getLeft());	//왼쪽 복사
			TreeNode R = treeCopy(T.getRight());	//오른쪽 복사
			S = T;	
			S.setLeft(T.getLeft());	//트리노드 S에 T의 얻어온 왼쪽값 으로 설정
			S.setRight(T.getRight());	//트리노드 S에 T의 얻오온 오른쪽값으로 설정
			S.setData(T.getData());		//트리노드 S에 T의 얻어온 데이터값으로 설정
		}
	return S;	//반환
}
